import { formatAmount, formatCurrency } from "@/pwa/shared/utils/format";
...
const QUICK_ACTIONS: QuickAction[] = [
  ...
  { icon: "pie_chart", label: "Categorias" },
  { icon: "event_repeat", label: "Recorrentes" },
  { icon: "group", label: "Comparar" },
];
        ...
        <div className="mt-6 pt-5 grid grid-cols-4 gap-2 bg-on-primary/5 rounded-2xl p-2.5 backdrop-blur-sm">
          {QUICK_ACTIONS.map((action) => (
            ...
            >
              ...
              <span className="mt-1.5 text-label-sm text-on-primary/90">{action.label}</span>
            </button>
          ))}
        </div>
      </div>
